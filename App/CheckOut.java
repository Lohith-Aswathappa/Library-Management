	...
	void prepareGUI()
	{
		...
		checkOut.addActionListener(new ActionListener()
		{
			...
			{if(isbnText.getText().equals("") || cardText.getText().equals("") )
			{
				...
				conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/library", "root", "sasi123");
				...
				if(rs1.next()&&rs2.next())
				{
					...
					ResultSet rs3 = stmt3.executeQuery("select * from library.book_loans where Isbn='"+isbn+"' and Date_in is null;");
					if(rs3.next())
					{
					...
					else
					{
						...
						ResultSet rs4 = stmt4.executeQuery("select * from library.book_loans where Card_id='"+cardId+"' and Date_in is null and Due_date<CAST(CURRENT_TIMESTAMP AS DATE);");
						if(rs4.next())
						{
						...
						else
						{
							...
							ResultSet rs5 = stmt5.executeQuery("select count(*) from library.book_loans where Card_id='"+cardId+"' and Date_in is null;");
							rs5.next();
							if(rs5.getInt(1)>=3)
							{
							...
							else
							{
								...
								ResultSet rs6 = stmt6.executeQuery("select * from library.book_loans b,fines f where Card_id="+cardId+" and b.Loan_id=f.Loan_id and paid=0;");	
								if(rs6.next())
								{
